...
class AttModel(nn.Module):
    def __init__(self):
        ...
        self.ctx2att = nn.Linear(self.rnn_size, self.att_h_size)
        self.ctx2pool = nn.Linear(self.rnn_size, self.att_h_size)
    ...
    def forward(self, img, seq, gt_seq, num, ppls, gt_boxes, mask_boxes, opt, eval_opt={}):
        if opt == 'MLE':
            return self.forward(img, seq, ppls, gt_boxes, mask_boxes, num)
        elif opt == 'RL':
            greedy_result = self._sample(torch.autograd.Variable(img.data, volatile=True),
                                        torch.autograd.Variable(ppls.data, volatile=True),
                                        torch.autograd.Variable(num.data, volatile=True),
            ...
            reward = torch.autograd.Variable(torch.from_numpy(reward).type_as(img.data).float())
            cider_score = torch.autograd.Variable(torch.Tensor(1).fill_(cider_score).type_as(img.data))
            lm_loss, bn_loss, fg_loss = self.critRL(gen_result[0], gen_result[1], gen_result[2],
                                                    gen_result[3], gen_result[4], gen_result[5], reward)
            ...
            return torch.autograd.Variable(seq), torch.autograd.Variable(bn_seq), torch.autograd.Variable(fg_seq)
        
    def init_hidden(self, bsz):
        weight = next(self.parameters()).data
        return (torch.autograd.Variable(weight.new(self.num_layers, bsz, self.rnn_size).zero()),
                torch.autograd.Variable(weight.new(self.num_layers, bsz, self.rnn_size).zero()))
    ...
    def _forward(self, img, seq, ppls, gt_boxes, mask_boxes, num):
        ...
        pnt_mask = torch.autograd.Variable(pnt_mask)
        ...
        if self.finetune_cnn:
            ...
            conv_feats, fc_feats = self.cnn(torch.autograd.Variable(img.data, volatile=True))
            conv_feats = torch.autograd.Variable(conv_feats.data)
            fc_feats = torch.autograd.Variable(fc_feats.data)
        ...
        pool_feats = self.roi_align(conv_feats)
        ...
        loc_feats = self.loc_fc(torch.autograd.Variable(loc_input))

        label_input = seq.data.new(batch_size, rois_num)
        label_input[:,:] = ppls.data[:,:,4]
        label_feat = self.det_fc(torch.autograd.Variable(label_input))
        ...
        fc_feats = fc_feats.view(batch_size, 1, self.fc_feat_size).expand(
                    batch_size, self.seq_per_img, self.fc_feat_size).contiguous().view(-1, self.fc_feat_size)
        ...
        det_output = nn.functional.log_softmax(det_output, dim=2)
        decoded = nn.functional.log_softmax(self.beta * self.logit(rnn_output), dim=2)
        ...
        roi_labels = torch.autograd.Variable(roi_labels)
        ...
        seq_update = torch.autograd.Variable(seq_update)
        ...
        vis_idx[vis_idx<0] = 0
        vis_idx = torch.autograd.Variable(vis_idx.view(-1))
    ...
    def _sample(self, img, ppls, num, opt={}):
        ...
        pool_feats = self.roi_align(conv_feats, torch.autograd.Variable(rois.view(-1,5)))
        pool_feats = pool_feats.view(batch_size, rois_num, self.att_feat_size)
        ...
        for t in range(self.seq_length + 1):
            if t == 0: # input <bos>
                ...
                sampleLogprobs = logprobs.gather(1, torch.autograd.Variable(it)) # gather the logprobs at sampled positions
            ...
            pnt_mask_list.append(torch.autograd.Variable(pnt_mask_new))
            ...
            xt = self.embed(torch.autograd.Variable(it_new))
            ...
            if t >= 1:
                ...
                bn_logprob, fg_logprob = self.ccr_core(vis_idx, pool_feats, \
                        rnn_output.view(batch_size,1,self.rnn_size), torch.autograd.Variable(roi_labels), batch_size, 1)
                ...
                if sample_max:
                    ...
                    slp_bn = bn_logprob.gather(1, torch.autograd.Variable(it_bn)) # gather the logprobs at sampled positions
                    slp_fg = fg_logprob.gather(1, torch.autograd.Variable(it_fg)) # gather the logprobs at sampled positions
    ...
    def _sample_beam(self, img, ppls, num, opt={}):
        ...
        for k in range(batch_size):
            ...
            xt = self.embed(torch.autograd.Variable(it))
